...
begin
  ...
  case getFormQueChamou of
    ...
    begin
      ...
      begin
        qryProcuradorSubstituido.Active := false;
        qryProcuradorSubstituido.Active := true;

        qryTotalDiasSubstituidos.Active := false;
...
function TfrmUpdateSubstituicao.excluirAfastamento(ID: String): boolean;
begin
  try
    with dmPessoal.qryExecSQL do
    begin
      ...
      SQL.Text := ' DELETE FROM tbSubstProcurador'
      + ' WHERE idSubstituicao = ' + QuotedStr(ID);
      ...
      ('ufrmUpdateSubstituicao', 'Exclusão de substituicao (excluirAfastamento)', SQL.Text);
    end;
    Result := true
