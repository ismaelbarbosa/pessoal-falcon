...
uses
  ...
  cxLookupEdit, cxDBLookupEdit, cxDBLookupComboBox, cxMemo, cxCalendar,
...
type
  TfrmUpdateFerias = class(TForm)
    ...
    rgpAbonoPecuniario: TcxRadioGroup;
    ...
    procedure rgpAbonoPecuniarioClick(Sender: TObject);
    procedure edtDtInicioPrimeiraPropertiesChange(Sender: TObject);
    procedure edtDtInicioSegundaPropertiesChange(Sender: TObject);
    procedure edtDtInicioTerceiraPropertiesChange(Sender: TObject);
    procedure lkpTipoParcelaPropertiesChange(Sender: TObject);
    procedure rgpParceladaPropertiesChange(Sender: TObject);
    procedure rgpPeriodoPropertiesChange(Sender: TObject);
    procedure rgpPeriodoExit(Sender: TObject);
  private
    OPERACAO: Integer;
    eProcurador: boolean;
  ...
  public
    ...
    procedure configurarOperacao_20220531;
    procedure configurarOperacao;

    // substituir a função retornarTipoParcela pelas duas abaixo dela
    //============================================================

    function retornarTipoParcela(idxParcela: Integer): boolean;

    function retornarTodosTiposParcela: Integer; // retornar todos os tipos
    function retornarTiposParcelaCategorizado: Integer; // retornar todos os tipos
    ...
    function carregarAbonoPecuniario:boolean;
    ...
    function jaExisteFerias
    (idPessoal, idServidor, idExercicio, idPeriodo: String):boolean;
    ...
    function habilitaGrupo(grupo: Integer; estado: boolean): boolean;
    function validarCampos: boolean;
    function jaExistePrimeiroPeriodo(idPessoal, idServidor, idExercicio: String):integer;
  end;
...
uses uDmExibirTabelas, PRG_utils, uPesFuncoes, uDmFeriasAbonoAfastamento,
  uDMPessoal, ufUpdateServidor, ufUpdateFeriasGeral, uDMConexao;
...
begin
  ...
  begin
    ...
    begin //2
      ...
      if FieldByName('idPeriodo').AsString = 'U' then // servidor
      begin
        //ShowMessage('período único');
        //lkpTipoParcela.Enabled := false;

        rgpPeriodo.ItemIndex := 0;
        habilitaGrupo(1, true);
        habilitaGrupo(2, false);
        habilitaGrupo(3, false);

        //ShowMessage('Período: ' + FieldByName('idPeriodo').AsString);
      end
      ...
      edtProcessoSEI.Text := FieldValues['nProcessoSEI']
      else edtProcessoSEI.Text := '';

      {
      if not FieldByName('Abono').IsNull then
      begin
        if FieldValues['Abono'] = false
        then
        begin
          //ShowMessage('Abono = não');
          rgpAbonoPecuniario.ItemIndex := 1;
        end
        else if FieldValues['Abono'] = true
        then
        begin
          //ShowMessage('Abono = sim');
          rgpAbonoPecuniario.ItemIndex := 0;
        end;
      end;
      }
      ...
      begin
        ...
        // o retorno da data final não será mais o que estava gravado
        // e sim o cálculo baseado no tipo de parcela
...
procedure TfrmUpdateFerias.configurarOperacao_20220531;
begin
  case OPERACAO of
    1:      // Incluir férias
    begin
  ...
  then
  begin
    rgpAbonoPecuniario.Enabled := true;

    //if jaExistePrimeiroPeriodo(idPessoal, idServidor, edtExercicio.Text) = 1 then
    //ShowMessage('Já existe o primeiro período');

    case OPERACAO of
      1: retornarTipoParcela(0); // Inclusão
      2: retornarTipoParcela(1); // Alteração
    end;
  ...
  begin
    rgpAbonoPecuniario.Enabled := false;
...
function TfrmUpdateFerias.retornaDataTermino(Data: TDate;
...
begin
  ...
  if (TipoParcela = 'U') and (rgpAbonoPecuniario.ItemIndex = 0)
  then aSomar := 19
  else if (TipoParcela = 'U') and (rgpAbonoPecuniario.ItemIndex = 1)
  then aSomar := 29
...
function TfrmUpdateFerias.retornarTipoParcela(
...
procedure TfrmUpdateFerias.setarDadosServidor(Matricula, Nome,
  Cargo: String);
begin
...
begin
  case modo of
    1: // habilita primeira
    begin
      gbxPrimeiraParcela.Visible := true;
    ...
    begin
      gbxPrimeiraParcela.Visible := false;
...
begin
  case modo of
    1: // habilita segunda
    begin
      gbxSegundaParcela.Visible := true;
    ...
    begin
      gbxSegundaParcela.Visible := false;
...
begin
  case modo of
    1: // habilita terceira
    begin
      gbxTerceiraParcela.Visible := true;
    ...
    begin
      gbxTerceiraParcela.Visible := false;
...
begin
  ...
  case OPERACAO of
    1: lkpTipoParcela.EditValue := 'U';
    2: rgpPeriodo.Enabled := false;
  end;
...
begin
  if not validarCampos then abort;
  ...
  begin
    ShowMessage('Informe a data de início da primeria parcela ou parcela única.');
...
function TfrmUpdateFerias.sqlDeleteFerias: String;
...
function TfrmUpdateFerias.sqlInsertFerias(parcela: Integer): String;
...
begin
  SQL_Ferias :=
      ...
      case rgpAbonoPecuniario.ItemIndex of
        0: SQL_Ferias := SQL_Ferias + QuotedStr('1');
        1: SQL_Ferias := SQL_Ferias + QuotedStr('0');
      end;
...
function TfrmUpdateFerias.sqlUpdateFerias: String;
var SQL_Ferias: String;
begin
  SQL_Ferias :=
      'SET DATEFORMAT dmy'
...
////
  ...
  lkpTipoParcelaPropertiesChange(Nil);
  //edtDtInicioPrimeiraPropertiesChange(Nil);
...
///
        ...
        if jaExisteFerias
        (idPessoal, idServidor, edtExercicio.Text, pPeriodo)
        then
        begin
          ...
          chr(13) + 'Será aberto agora para edição';
...
begin
  ...
  begin
    ...
    SQL.Text := 'SELECT top 1 * from tbFerias'
              ...
              + ' AND idPeriodo = ' + QuotedStr(idPeriodo);
...
procedure TfrmUpdateFerias.rgpAbonoPecuniarioClick(Sender: TObject);
begin
  if OPERACAO = 2      // Alterar férias
  then
  lblDt_TerminoPrimeira.Caption := DateToStr
  (retornaDataTermino(StrToDate(edtDtInicioPrimeira.Text), lkpTipoParcela.EditValue, 1));
end;

procedure TfrmUpdateFerias.edtDtInicioPrimeiraPropertiesChange(
  Sender: TObject);
begin
  if
  //(edtDtInicioPrimeira.Focused) and
  (Length(edtDtInicioPrimeira.Text) = 10)
  then
  lblDt_TerminoPrimeira.Caption := DateToStr
  (retornaDataTermino(StrToDate(edtDtInicioPrimeira.Text), lkpTipoParcela.EditValue, 1));
end;

function TfrmUpdateFerias.carregarAbonoPecuniario: boolean;
begin
  with dmFeriasAbonoAfastamento.qryFeriasNoPeriodo do
  begin
    Open;
...
procedure TfrmUpdateFerias.edtDtInicioSegundaPropertiesChange(
  Sender: TObject);
begin
  if
  //(edtDtInicioSegunda.Focused) and
  (Length(edtDtInicioSegunda.Text) = 10)
  then
    lblDt_TerminoSegunda.Caption := DateToStr
    (retornaDataTermino(StrToDate(edtDtInicioSegunda.Text), lkpTipoParcela.EditValue, 2));
end;

procedure TfrmUpdateFerias.edtDtInicioTerceiraPropertiesChange(
  Sender: TObject);
begin
  if
  //(edtDtInicioTerceira.Focused) and
  (Length(edtDtInicioTerceira.Text) = 10)
  then
    lblDt_TerminoTerceira.Caption := DateToStr
    (retornaDataTermino(StrToDate(edtDtInicioTerceira.Text), lkpTipoParcela.EditValue, 2));
end;

function TfrmUpdateFerias.habilitaGrupo(grupo: Integer; estado: boolean): boolean;
begin
  case grupo of
    1: gbxPrimeiraParcela.Visible := estado;
    2: gbxSegundaParcela.Visible := estado;
    3: gbxTerceiraParcela.Visible := estado;
  end;
end;

procedure TfrmUpdateFerias.lkpTipoParcelaPropertiesChange(Sender: TObject);
begin
  if lkpTipoParcela.EditValue = 'U' then
  begin
    habilitaGrupo(2, false);
  ...
  edtDtInicioPrimeiraPropertiesChange(Nil);
  edtDtInicioSegundaPropertiesChange(Nil);
  edtDtInicioTerceiraPropertiesChange(Nil);
end;

procedure TfrmUpdateFerias.rgpParceladaPropertiesChange(Sender: TObject);
begin
//  ShowMessage('Valor alterado');


  case rgpParcelada.ItemIndex of
    0: // parcelada
    begin
      // não é permitido abono pecuniário para férias parceladas
      lkpTipoParcela.Enabled := true;
...
////
  ...
  // adicionadas as linhas abaixo em 30/05/2022
...
function TfrmUpdateFerias.validarCampos: boolean;
begin
  if edtExercicio.Text = '' then
  begin
    ShowMessage('Informe o exercício');
  ...
  if (rgpParcelada.ItemIndex = 0) then
  begin
    if (lkpTipoParcela.EditValue = 'D') and (Trim(edtDtInicioTerceira.Text) = '')
    then
    begin
      ShowMessage('Informe a data de início da terceira parcela.');
      Result := false;
    ...
    if (Trim(edtDtInicioSegunda.Text) = '') then
    begin
      ShowMessage('Informe a data de início da segunda parcela.');
      Result := false;
      Abort;
...
function TfrmUpdateFerias.jaExistePrimeiroPeriodo(idPessoal, idServidor,
  idExercicio: String): integer;
begin
  with dmPessoal.qryExecSQL do
  begin
    Active := false;
    ...
    Active := true;
    Result := dmPessoal.qryExecSQL.RecordCount;
  end
end;

procedure TfrmUpdateFerias.rgpPeriodoPropertiesChange(Sender: TObject);
var pPeriodo, wMensagem: String;
begin
  case rgpPeriodo.ItemIndex of
    0: pPeriodo := 'U';
    1: pPeriodo := '1';
...
procedure TfrmUpdateFerias.rgpPeriodoExit(Sender: TObject);
begin
  rgpPeriodoPropertiesChange(Nil);

end;

function TfrmUpdateFerias.retornarTiposParcelaCategorizado: Integer;
begin
//
end;

function TfrmUpdateFerias.retornarTodosTiposParcela: Integer;
var wSQL: String;
begin
  with dmExibirTabelas do
  begin
    try
      ...
      monitorarAcoesDaSessao('ufUpdateFerias', 'retornarTodosTiposParcela',
      wSQL);
    ...
    Result := qryFeriasTipoParcela.RecordCount;
  end;
end;
