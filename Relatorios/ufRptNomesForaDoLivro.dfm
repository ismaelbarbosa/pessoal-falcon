object frmRptNomesForaDoLivro: TfrmRptNomesForaDoLivro
  Left = 192
  Top = 117
  Width = 1044
  Height = 676
  Caption = 'frmRptNomesForaDoLivro'
  Color = clBtnFace
  Font.Charset = ANSI_CHARSET
  Font.Color = clWindowText
  Font.Height = -13
  Font.Name = 'Verdana'
  Font.Style = []
  OldCreateOrder = False
  Scaled = False
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 16
  object pnlBotoes: TPanel
    Left = 0
    Top = 0
    Width = 1036
    Height = 44
    Align = alTop
    BevelOuter = bvNone
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Verdana'
    Font.Style = []
    ParentFont = False
    TabOrder = 0
    object btnSair: TcxButton
      Left = 257
      Top = 5
      Width = 110
      Height = 35
      Caption = 'Sai&r'
      TabOrder = 1
      OnClick = btnSairClick
      Glyph.Data = {
        
        000000000000}
    end
    object btnPesquisar: TcxButton
      Left = 7
      Top = 5
      Width = 110
      Height = 35
      Caption = '&Pesquisar'
      TabOrder = 0
      Glyph.Data = {
        
        000000000000}
    end
    object btnImprimir: TcxButton
      Left = 121
      Top = 5
      Width = 110
      Height = 35
      Caption = 'Imprimir'
      TabOrder = 2
      OnClick = btnImprimirClick
      Glyph.Data = {
        
        000000000000}
    end
  end
  object cxPageControl1: TcxPageControl
    Left = 0
    Top = 44
    Width = 1036
    Height = 601
    ActivePage = tshPesquisa
    Align = alClient
    TabOrder = 1
    ClientRectBottom = 601
    ClientRectRight = 1036
    ClientRectTop = 27
    object tshPesquisa: TcxTabSheet
      Caption = 'Pesquisa'
      ImageIndex = 0
      object cxGrid1: TcxGrid
        Left = 0
        Top = 410
        Width = 1036
        Height = 105
        Align = alTop
        TabOrder = 0
        object cxGrid1DBTableView1: TcxGridDBTableView
          NavigatorButtons.ConfirmDelete = False
          DataController.Summary.DefaultGroupSummaryItems = <>
          DataController.Summary.FooterSummaryItems = <>
          DataController.Summary.SummaryGroups = <>
          object cxGrid1DBTableView1Column1: TcxGridDBColumn
            DataBinding.FieldName = 'dt_Lotacao'
          end
          object cxGrid1DBTableView1Column2: TcxGridDBColumn
            DataBinding.FieldName = 'Lotacao'
            Width = 223
          end
        end
        object cxGrid1Level1: TcxGridLevel
          GridView = cxGrid1DBTableView1
        end
      end
      object cxGrid2: TcxGrid
        Left = 0
        Top = 0
        Width = 1036
        Height = 200
        Align = alTop
        TabOrder = 1
        object cxGridDBTableView1: TcxGridDBTableView
          NavigatorButtons.ConfirmDelete = False
          DataController.DataSource = dsPessoal
          DataController.Summary.DefaultGroupSummaryItems = <>
          DataController.Summary.FooterSummaryItems = <>
          DataController.Summary.SummaryGroups = <>
          object cxGridDBColumn1: TcxGridDBColumn
            DataBinding.FieldName = 'idPessoal'
          end
          object cxGridDBColumn2: TcxGridDBColumn
            DataBinding.FieldName = 'Nome'
          end
          object cxGridDBColumn3: TcxGridDBColumn
            DataBinding.FieldName = 'Dt_Nascimento'
          end
        end
        object cxGridLevel1: TcxGridLevel
          GridView = cxGridDBTableView1
        end
      end
      object cxGrid3: TcxGrid
        Left = 0
        Top = 200
        Width = 1036
        Height = 105
        Align = alTop
        TabOrder = 2
        object cxGridDBTableView2: TcxGridDBTableView
          NavigatorButtons.ConfirmDelete = False
          DataController.DataSource = dsServidor
          DataController.Summary.DefaultGroupSummaryItems = <>
          DataController.Summary.FooterSummaryItems = <>
          DataController.Summary.SummaryGroups = <>
          object cxGridDBColumn4: TcxGridDBColumn
            DataBinding.FieldName = 'idServidor'
          end
          object cxGridDBColumn5: TcxGridDBColumn
            DataBinding.FieldName = 'Cargo'
            Width = 223
          end
        end
        object cxGridLevel2: TcxGridLevel
          GridView = cxGridDBTableView2
        end
      end
      object cxGrid4: TcxGrid
        Left = 0
        Top = 305
        Width = 1036
        Height = 105
        Align = alTop
        TabOrder = 3
        object cxGridDBTableView3: TcxGridDBTableView
          NavigatorButtons.ConfirmDelete = False
          DataController.Summary.DefaultGroupSummaryItems = <>
          DataController.Summary.FooterSummaryItems = <>
          DataController.Summary.SummaryGroups = <>
          object cxGridDBColumn6: TcxGridDBColumn
            DataBinding.FieldName = 'dt_NomeacaoFuncao'
          end
          object cxGridDBColumn7: TcxGridDBColumn
            DataBinding.FieldName = 'Funcao'
            Width = 223
          end
        end
        object cxGridLevel3: TcxGridLevel
          GridView = cxGridDBTableView3
        end
      end
    end
    object tshRelatorio: TcxTabSheet
      Caption = 'Relat'#243'rio'
      ImageIndex = 1
      object QuickRep1: TQuickRep
        Left = 56
        Top = 32
        Width = 794
        Height = 1123
        Frame.Color = clBlack
        Frame.DrawTop = False
        Frame.DrawBottom = False
        Frame.DrawLeft = False
        Frame.DrawRight = False
        DataSet = qryPessoal
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'Arial'
        Font.Style = []
        Functions.Strings = (
          'PAGENUMBER'
          'COLUMNNUMBER'
          'REPORTTITLE')
        Functions.DATA = (
          '0'
          '0'
          #39#39)
        Options = [FirstPageHeader, LastPageFooter]
        Page.Columns = 1
        Page.Orientation = poPortrait
        Page.PaperSize = A4
        Page.Values = (
          100.000000000000000000
          2970.000000000000000000
          100.000000000000000000
          2100.000000000000000000
          100.000000000000000000
          100.000000000000000000
          0.000000000000000000)
        PrinterSettings.Copies = 1
        PrinterSettings.Duplex = False
        PrinterSettings.FirstPage = 0
        PrinterSettings.LastPage = 0
        PrinterSettings.OutputBin = Auto
        PrintIfEmpty = True
        SnapToGrid = True
        Units = MM
        Zoom = 100
        object DetailBand1: TQRBand
          Left = 38
          Top = 89
          Width = 718
          Height = 109
          Frame.Color = clBlack
          Frame.DrawTop = True
          Frame.DrawBottom = False
          Frame.DrawLeft = False
          Frame.DrawRight = False
          AlignToBottom = False
          Color = clWhite
          ForceNewColumn = False
          ForceNewPage = False
          Size.Values = (
            288.395833333333300000
            1899.708333333333000000)
          BandType = rbGroupHeader
          object QRDBText1: TQRDBText
            Left = 8
            Top = 20
            Width = 353
            Height = 23
            Frame.Color = clBlack
            Frame.DrawTop = False
            Frame.DrawBottom = False
            Frame.DrawLeft = False
            Frame.DrawRight = False
            Size.Values = (
              60.854166666666670000
              21.166666666666670000
              52.916666666666670000
              933.979166666666700000)
            Alignment = taLeftJustify
            AlignToBand = False
            AutoSize = False
            AutoStretch = False
            Color = clWhite
            DataSet = qryPessoal
            DataField = 'Nome'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -19
            Font.Name = 'Arial'
            Font.Style = [fsBold]
            ParentFont = False
            Transparent = False
            WordWrap = True
            FontSize = 14
          end
          object QRDBText9: TQRDBText
            Left = 368
            Top = 27
            Width = 142
            Height = 17
            Frame.Color = clBlack
            Frame.DrawTop = False
            Frame.DrawBottom = False
            Frame.DrawLeft = False
            Frame.DrawRight = False
            Size.Values = (
              44.979166666666670000
              973.666666666666700000
              71.437500000000000000
              375.708333333333300000)
            Alignment = taLeftJustify
            AlignToBand = False
            AutoSize = False
            AutoStretch = False
            Color = clWhite
            DataSet = qryPessoal
            DataField = 'CPF'
            Transparent = False
            WordWrap = True
            FontSize = 10
          end
          object QRDBText10: TQRDBText
            Left = 536
            Top = 27
            Width = 142
            Height = 17
            Frame.Color = clBlack
            Frame.DrawTop = False
            Frame.DrawBottom = False
            Frame.DrawLeft = False
            Frame.DrawRight = False
            Size.Values = (
              44.979166666666670000
              1418.166666666667000000
              71.437500000000000000
              375.708333333333300000)
            Alignment = taLeftJustify
            AlignToBand = False
            AutoSize = False
            AutoStretch = False
            Color = clWhite
            DataSet = qryPessoal
            DataField = 'Dt_Nascimento'
            Transparent = False
            WordWrap = True
            FontSize = 10
          end
          object QRLabel9: TQRLabel
            Left = 352
            Top = 56
            Width = 22
            Height = 17
            Frame.Color = clBlack
            Frame.DrawTop = False
            Frame.DrawBottom = False
            Frame.DrawLeft = False
            Frame.DrawRight = False
            Size.Values = (
              44.979166666666670000
              931.333333333333300000
              148.166666666666700000
              58.208333333333330000)
            Alignment = taLeftJustify
            AlignToBand = False
            AutoSize = True
            AutoStretch = False
            Caption = 'PAI'
            Color = clWhite
            Transparent = False
            WordWrap = True
            FontSize = 10
          end
          object QRLabel10: TQRLabel
            Left = 8
            Top = 56
            Width = 30
            Height = 17
            Frame.Color = clBlack
            Frame.DrawTop = False
            Frame.DrawBottom = False
            Frame.DrawLeft = False
            Frame.DrawRight = False
            Size.Values = (
              44.979166666666670000
              21.166666666666670000
              148.166666666666700000
              79.375000000000000000)
            Alignment = taLeftJustify
            AlignToBand = False
            AutoSize = True
            AutoStretch = False
            Caption = 'M'#195'E'
            Color = clWhite
            Transparent = False
            WordWrap = True
            FontSize = 10
          end
          object QRDBText6: TQRDBText
            Left = 381
            Top = 56
            Width = 20
            Height = 17
            Frame.Color = clBlack
            Frame.DrawTop = False
            Frame.DrawBottom = False
            Frame.DrawLeft = False
            Frame.DrawRight = False
            Size.Values = (
              44.979166666666670000
              1008.062500000000000000
              148.166666666666700000
              52.916666666666670000)
            Alignment = taLeftJustify
            AlignToBand = False
            AutoSize = True
            AutoStretch = False
            Color = clWhite
            DataSet = qryPessoal
            DataField = 'Pai'
            Transparent = False
            WordWrap = True
            FontSize = 10
          end
          object QRDBText7: TQRDBText
            Left = 48
            Top = 56
            Width = 26
            Height = 17
            Frame.Color = clBlack
            Frame.DrawTop = False
            Frame.DrawBottom = False
            Frame.DrawLeft = False
            Frame.DrawRight = False
            Size.Values = (
              44.979166666666670000
              127.000000000000000000
              148.166666666666700000
              68.791666666666670000)
            Alignment = taLeftJustify
            AlignToBand = False
            AutoSize = True
            AutoStretch = False
            Color = clWhite
            DataSet = qryPessoal
            DataField = 'Mae'
            Transparent = False
            WordWrap = True
            FontSize = 10
          end
          object QRLabel5: TQRLabel
            Left = 368
            Top = 8
            Width = 27
            Height = 17
            Frame.Color = clBlack
            Frame.DrawTop = False
            Frame.DrawBottom = False
            Frame.DrawLeft = False
            Frame.DrawRight = False
            Size.Values = (
              44.979166666666670000
              973.666666666666700000
              21.166666666666670000
              71.437500000000000000)
            Alignment = taLeftJustify
            AlignToBand = False
            AutoSize = True
            AutoStretch = False
            Caption = 'CPF'
            Color = clWhite
            Transparent = False
            WordWrap = True
            FontSize = 10
          end
          object QRLabel6: TQRLabel
            Left = 536
            Top = 8
            Width = 146
            Height = 17
            Frame.Color = clBlack
            Frame.DrawTop = False
            Frame.DrawBottom = False
            Frame.DrawLeft = False
            Frame.DrawRight = False
            Size.Values = (
              44.979166666666670000
              1418.166666666667000000
              21.166666666666670000
              386.291666666666700000)
            Alignment = taLeftJustify
            AlignToBand = False
            AutoSize = True
            AutoStretch = False
            Caption = 'DATA DE NASCIMENTO'
            Color = clWhite
            Transparent = False
            WordWrap = True
            FontSize = 10
          end
          object QRLabel11: TQRLabel
            Left = 8
            Top = 72
            Width = 63
            Height = 17
            Frame.Color = clBlack
            Frame.DrawTop = False
            Frame.DrawBottom = False
            Frame.DrawLeft = False
            Frame.DrawRight = False
            Size.Values = (
              44.979166666666670000
              21.166666666666670000
              190.500000000000000000
              166.687500000000000000)
            Alignment = taLeftJustify
            AlignToBand = False
            AutoSize = True
            AutoStretch = False
            Caption = 'CONJUGE'
            Color = clWhite
            Transparent = False
            WordWrap = True
            FontSize = 10
          end
          object QRDBText8: TQRDBText
            Left = 75
            Top = 72
            Width = 48
            Height = 17
            Frame.Color = clBlack
            Frame.DrawTop = False
            Frame.DrawBottom = False
            Frame.DrawLeft = False
            Frame.DrawRight = False
            Size.Values = (
              44.979166666666670000
              198.437500000000000000
              190.500000000000000000
              127.000000000000000000)
            Alignment = taLeftJustify
            AlignToBand = False
            AutoSize = True
            AutoStretch = False
            Color = clWhite
            DataSet = qryPessoal
            DataField = 'Conjuge'
            Transparent = False
            WordWrap = True
            FontSize = 10
          end
          object QRLabel2: TQRLabel
            Left = 8
            Top = 88
            Width = 74
            Height = 17
            Frame.Color = clBlack
            Frame.DrawTop = False
            Frame.DrawBottom = False
            Frame.DrawLeft = False
            Frame.DrawRight = False
            Size.Values = (
              44.979166666666670000
              21.166666666666670000
              232.833333333333300000
              195.791666666666700000)
            Alignment = taLeftJustify
            AlignToBand = False
            AutoSize = True
            AutoStretch = False
            Caption = 'ENDERE'#199'O'
            Color = clWhite
            Transparent = False
            WordWrap = True
            FontSize = 10
          end
          object QRDBText11: TQRDBText
            Left = 88
            Top = 88
            Width = 56
            Height = 17
            Frame.Color = clBlack
            Frame.DrawTop = False
            Frame.DrawBottom = False
            Frame.DrawLeft = False
            Frame.DrawRight = False
            Size.Values = (
              44.979166666666670000
              232.833333333333300000
              232.833333333333300000
              148.166666666666700000)
            Alignment = taLeftJustify
            AlignToBand = False
            AutoSize = True
            AutoStretch = False
            Color = clWhite
            DataSet = qryPessoal
            DataField = 'Endereco'
            Transparent = False
            WordWrap = True
            FontSize = 10
          end
          object QRLabel12: TQRLabel
            Left = 432
            Top = 88
            Width = 49
            Height = 17
            Frame.Color = clBlack
            Frame.DrawTop = False
            Frame.DrawBottom = False
            Frame.DrawLeft = False
            Frame.DrawRight = False
            Size.Values = (
              44.979166666666670000
              1143.000000000000000000
              232.833333333333300000
              129.645833333333300000)
            Alignment = taLeftJustify
            AlignToBand = False
            AutoSize = True
            AutoStretch = False
            Caption = 'CIDADE'
            Color = clWhite
            Transparent = False
            WordWrap = True
            FontSize = 10
          end
          object QRDBText12: TQRDBText
            Left = 488
            Top = 88
            Width = 41
            Height = 17
            Frame.Color = clBlack
            Frame.DrawTop = False
            Frame.DrawBottom = False
            Frame.DrawLeft = False
            Frame.DrawRight = False
            Size.Values = (
              44.979166666666670000
              1291.166666666667000000
              232.833333333333300000
              108.479166666666700000)
            Alignment = taLeftJustify
            AlignToBand = False
            AutoSize = True
            AutoStretch = False
            Color = clWhite
            DataSet = qryPessoal
            DataField = 'Cidade'
            Transparent = False
            WordWrap = True
            FontSize = 10
          end
          object QRLabel13: TQRLabel
            Left = 656
            Top = 88
            Width = 18
            Height = 17
            Frame.Color = clBlack
            Frame.DrawTop = False
            Frame.DrawBottom = False
            Frame.DrawLeft = False
            Frame.DrawRight = False
            Size.Values = (
              44.979166666666670000
              1735.666666666667000000
              232.833333333333300000
              47.625000000000000000)
            Alignment = taLeftJustify
            AlignToBand = False
            AutoSize = True
            AutoStretch = False
            Caption = 'UF'
            Color = clWhite
            Transparent = False
            WordWrap = True
            FontSize = 10
          end
          object QRDBText13: TQRDBText
            Left = 680
            Top = 88
            Width = 18
            Height = 17
            Frame.Color = clBlack
            Frame.DrawTop = False
            Frame.DrawBottom = False
            Frame.DrawLeft = False
            Frame.DrawRight = False
            Size.Values = (
              44.979166666666670000
              1799.166666666667000000
              232.833333333333300000
              47.625000000000000000)
            Alignment = taLeftJustify
            AlignToBand = False
            AutoSize = True
            AutoStretch = False
            Color = clWhite
            DataSet = qryPessoal
            DataField = 'UF'
            Transparent = False
            WordWrap = True
            FontSize = 10
          end
        end
        object PageFooterBand1: TQRBand
          Left = 38
          Top = 262
          Width = 718
          Height = 40
          Frame.Color = clBlack
          Frame.DrawTop = False
          Frame.DrawBottom = False
          Frame.DrawLeft = False
          Frame.DrawRight = False
          AlignToBottom = False
          Color = clWhite
          ForceNewColumn = False
          ForceNewPage = False
          Size.Values = (
            105.833333333333300000
            1899.708333333333000000)
          BandType = rbPageFooter
          object QRSysData1: TQRSysData
            Left = 320
            Top = 8
            Width = 46
            Height = 17
            Frame.Color = clBlack
            Frame.DrawTop = False
            Frame.DrawBottom = False
            Frame.DrawLeft = False
            Frame.DrawRight = False
            Size.Values = (
              44.979166666666670000
              846.666666666666700000
              21.166666666666670000
              121.708333333333300000)
            Alignment = taLeftJustify
            AlignToBand = False
            AutoSize = True
            Color = clWhite
            Data = qrsPageNumber
            Transparent = False
            FontSize = 10
          end
        end
        object PageHeaderBand1: TQRBand
          Left = 38
          Top = 38
          Width = 718
          Height = 11
          Frame.Color = clBlack
          Frame.DrawTop = False
          Frame.DrawBottom = False
          Frame.DrawLeft = False
          Frame.DrawRight = False
          AlignToBottom = False
          Color = clWhite
          ForceNewColumn = False
          ForceNewPage = False
          Size.Values = (
            29.104166666666670000
            1899.708333333333000000)
          BandType = rbPageHeader
        end
        object TitleBand1: TQRBand
          Left = 38
          Top = 49
          Width = 718
          Height = 40
          Frame.Color = clBlack
          Frame.DrawTop = False
          Frame.DrawBottom = False
          Frame.DrawLeft = False
          Frame.DrawRight = False
          AlignToBottom = False
          Color = clWhite
          ForceNewColumn = False
          ForceNewPage = False
          Size.Values = (
            105.833333333333300000
            1899.708333333333000000)
          BandType = rbTitle
          object QRLabel1: TQRLabel
            Left = 8
            Top = 8
            Width = 584
            Height = 25
            Frame.Color = clBlack
            Frame.DrawTop = False
            Frame.DrawBottom = False
            Frame.DrawLeft = False
            Frame.DrawRight = False
            Size.Values = (
              66.145833333333330000
              21.166666666666670000
              21.166666666666670000
              1545.166666666667000000)
            Alignment = taLeftJustify
            AlignToBand = False
            AutoSize = True
            AutoStretch = False
            Caption = 'RELA'#199#195'O DE PROCURADORES QUE N'#195'O CONSTAM NO LIVRO'
            Color = clWhite
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -19
            Font.Name = 'Arial'
            Font.Style = [fsBold]
            ParentFont = False
            Transparent = False
            WordWrap = True
            FontSize = 14
          end
        end
        object QRSubDetail1: TQRSubDetail
          Left = 38
          Top = 198
          Width = 718
          Height = 64
          Frame.Color = clBlack
          Frame.DrawTop = False
          Frame.DrawBottom = True
          Frame.DrawLeft = False
          Frame.DrawRight = False
          AlignToBottom = False
          Color = clWhite
          ForceNewColumn = False
          ForceNewPage = False
          LinkBand = DetailBand1
          Size.Values = (
            169.333333333333300000
            1899.708333333333000000)
          Master = QuickRep1
          DataSet = qryServidor
          HeaderBand = DetailBand1
          PrintBefore = False
          PrintIfEmpty = True
          object QRDBText2: TQRDBText
            Left = 90
            Top = 8
            Width = 57
            Height = 17
            Frame.Color = clBlack
            Frame.DrawTop = False
            Frame.DrawBottom = False
            Frame.DrawLeft = False
            Frame.DrawRight = False
            Size.Values = (
              44.979166666666670000
              238.125000000000000000
              21.166666666666670000
              150.812500000000000000)
            Alignment = taLeftJustify
            AlignToBand = False
            AutoSize = True
            AutoStretch = False
            Color = clWhite
            DataSet = qryServidor
            DataField = 'idServidor'
            Transparent = False
            WordWrap = True
            FontSize = 10
          end
          object QRDBText3: TQRDBText
            Left = 232
            Top = 8
            Width = 35
            Height = 17
            Frame.Color = clBlack
            Frame.DrawTop = False
            Frame.DrawBottom = False
            Frame.DrawLeft = False
            Frame.DrawRight = False
            Size.Values = (
              44.979166666666670000
              613.833333333333300000
              21.166666666666670000
              92.604166666666670000)
            Alignment = taLeftJustify
            AlignToBand = False
            AutoSize = True
            AutoStretch = False
            Color = clWhite
            DataSet = qryServidor
            DataField = 'Cargo'
            Transparent = False
            WordWrap = True
            FontSize = 10
          end
          object QRDBText4: TQRDBText
            Left = 88
            Top = 24
            Width = 47
            Height = 17
            Frame.Color = clBlack
            Frame.DrawTop = False
            Frame.DrawBottom = False
            Frame.DrawLeft = False
            Frame.DrawRight = False
            Size.Values = (
              44.979166666666670000
              232.833333333333300000
              63.500000000000000000
              124.354166666666700000)
            Alignment = taLeftJustify
            AlignToBand = False
            AutoSize = True
            AutoStretch = False
            Color = clWhite
            DataSet = qryServidor
            DataField = 'Lotacao'
            Transparent = False
            WordWrap = True
            FontSize = 10
          end
          object QRLabel3: TQRLabel
            Left = 8
            Top = 8
            Width = 78
            Height = 17
            Frame.Color = clBlack
            Frame.DrawTop = False
            Frame.DrawBottom = False
            Frame.DrawLeft = False
            Frame.DrawRight = False
            Size.Values = (
              44.979166666666670000
              21.166666666666670000
              21.166666666666670000
              206.375000000000000000)
            Alignment = taLeftJustify
            AlignToBand = False
            AutoSize = True
            AutoStretch = False
            Caption = 'MATR'#205'CULA:'
            Color = clWhite
            Transparent = False
            WordWrap = True
            FontSize = 10
          end
          object QRLabel4: TQRLabel
            Left = 176
            Top = 8
            Width = 52
            Height = 17
            Frame.Color = clBlack
            Frame.DrawTop = False
            Frame.DrawBottom = False
            Frame.DrawLeft = False
            Frame.DrawRight = False
            Size.Values = (
              44.979166666666670000
              465.666666666666700000
              21.166666666666670000
              137.583333333333300000)
            Alignment = taLeftJustify
            AlignToBand = False
            AutoSize = True
            AutoStretch = False
            Caption = 'CARGO:'
            Color = clWhite
            Transparent = False
            WordWrap = True
            FontSize = 10
          end
          object QRLabel7: TQRLabel
            Left = 8
            Top = 24
            Width = 66
            Height = 17
            Frame.Color = clBlack
            Frame.DrawTop = False
            Frame.DrawBottom = False
            Frame.DrawLeft = False
            Frame.DrawRight = False
            Size.Values = (
              44.979166666666670000
              21.166666666666670000
              63.500000000000000000
              174.625000000000000000)
            Alignment = taLeftJustify
            AlignToBand = False
            AutoSize = True
            AutoStretch = False
            Caption = 'LOTA'#199#195'O:'
            Color = clWhite
            Transparent = False
            WordWrap = True
            FontSize = 10
          end
          object QRDBText5: TQRDBText
            Left = 120
            Top = 40
            Width = 80
            Height = 17
            Frame.Color = clBlack
            Frame.DrawTop = False
            Frame.DrawBottom = False
            Frame.DrawLeft = False
            Frame.DrawRight = False
            Size.Values = (
              44.979166666666670000
              317.500000000000000000
              105.833333333333300000
              211.666666666666700000)
            Alignment = taLeftJustify
            AlignToBand = False
            AutoSize = True
            AutoStretch = False
            Color = clWhite
            DataSet = qryServidor
            DataField = 'Desligamento'
            Transparent = False
            WordWrap = True
            FontSize = 10
          end
          object QRLabel8: TQRLabel
            Left = 8
            Top = 40
            Width = 103
            Height = 17
            Frame.Color = clBlack
            Frame.DrawTop = False
            Frame.DrawBottom = False
            Frame.DrawLeft = False
            Frame.DrawRight = False
            Size.Values = (
              44.979166666666670000
              21.166666666666670000
              105.833333333333300000
              272.520833333333300000)
            Alignment = taLeftJustify
            AlignToBand = False
            AutoSize = True
            AutoStretch = False
            Caption = 'DESLIGAMENTO'
            Color = clWhite
            Transparent = False
            WordWrap = True
            FontSize = 10
          end
        end
      end
    end
  end
  object qryPessoal: TADOQuery
    Connection = DMConexao.conPessoal
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      'SELECT'
      'pes.idPessoal, Nome, CPF, dt_Nascimento,'
      'dad.Naturalidade,'
      'dad.Pai, dad.Mae,'
      'dad.Conjuge,'
      'dad.Endereco,'
      'dad.Cidade,'
      'dad.UF'
      ''
      'FROM  tbPessoal pes'
      'LEFT JOIN tbDados dad on dad.idPessoal = pes.idPessoal'
      'WHERE nome IN'
      '('
      #39'ARNALDO REINERT'#39','
      #39'ATALIBA TAVARES NOGUEIRA'#39','
      #39'BRASIL COURY'#39','
      #39'CARLOS FERNANDO MATHIAS DE SOUZA'#39','
      #39'CIRLAINE LEITCO BARROSO'#39','
      #39'CIRZA DE MORAES DINIZ'#39','
      #39'EDISON MOTA DA SIVA'#39','
      #39'FRANCISCO LUIZ DE BESSA LEITE'#39','
      #39'HELION MOREIRA SILVA'#39','
      #39'IEDA ALBUQUERQUE'#39','
      #39'ILDEU DINIZ'#39','
      #39'JOAO BAEZA'#39','
      #39'JORES CARLOS ALVES DOS SANTOS'#39','
      #39'JOSE EDUARDO PIRES CAMPOS'#39','
      #39'JOSE MARIA ANTUNES TOLENTINO'#39','
      #39'JOSE MARIA PELUCIO PEREIRA'#39','
      #39'JULIO CESAR DE ROSE'#39','
      #39'LISIA BARREIRA MONIZ DE ARAGAO'#39','
      #39'MAGALY BALDUINO DE S. MILHOMENS'#39','
      #39'MANOEL CESAR NETO'#39','
      #39'MARCIO HUMMEL DE CASTRO'#39','
      #39'MARIA NAZARE LIMA MASCARENHAS'#39','
      #39'MARIA NILZE PARREIRA'#39','
      #39'NAGIB ALVES GANEN'#39','
      #39'NAZARENO ALVES SOBRINHO'#39','
      #39'NEIL DIAS ABRAHCO'#39','
      #39'OCELIO DE MEDEIROS'#39','
      #39'OCTAVIO LEITE DE SOUZA'#39','
      #39'ORLANDO MIRANDA DE ARAGAO'#39','
      #39'PEDRITA MARIA BRAILE PEREIRA'#39','
      #39'RAIMUNDO ALVES CORDEIRO'#39','
      #39'RICARDO JOSE MACEDO DE BRITTO PEREIRA'#39','
      #39'ROBERTO JORGE DINO'#39','
      #39'SEBASTIAO OSCAR DE CASTRO'#39','
      #39'SERGIO DA COSTA RIBEIRO'#39','
      #39'SILVIA HELENA AMORIM CORONEL'#39','
      #39'SIMONE NOBREGA DE ALMEIDA LINS'#39','
      #39'WALDIVINO CARVALHO DOS SANTOS'#39','
      #39'WALKYRIO RODRIGUES COELHO'#39','
      #39'YOLANDA HUDSON RAMIREZ'#39
      ')')
    Left = 488
    Top = 8
    object qryPessoalidPessoal: TStringField
      FieldName = 'idPessoal'
      FixedChar = True
      Size = 5
    end
    object qryPessoalNome: TStringField
      FieldName = 'Nome'
      Size = 60
    end
    object qryPessoalDt_Nascimento: TDateTimeField
      FieldName = 'Dt_Nascimento'
    end
    object qryPessoalCPF: TStringField
      FieldName = 'CPF'
      FixedChar = True
      Size = 11
    end
    object qryPessoalNaturalidade: TStringField
      FieldName = 'Naturalidade'
      Size = 30
    end
    object qryPessoalPai: TStringField
      FieldName = 'Pai'
      Size = 50
    end
    object qryPessoalMae: TStringField
      FieldName = 'Mae'
      Size = 50
    end
    object qryPessoalConjuge: TStringField
      FieldName = 'Conjuge'
      Size = 50
    end
    object qryPessoalEndereco: TStringField
      FieldName = 'Endereco'
      Size = 50
    end
    object qryPessoalCidade: TStringField
      FieldName = 'Cidade'
      Size = 30
    end
    object qryPessoalUF: TStringField
      FieldName = 'UF'
      FixedChar = True
      Size = 2
    end
  end
  object dsPessoal: TDataSource
    DataSet = qryPessoal
    Left = 520
    Top = 8
  end
  object qryServidor: TADOQuery
    Connection = DMConexao.conPessoal
    CursorType = ctStatic
    DataSource = dsPessoal
    Parameters = <
      item
        Name = 'idPessoal'
        DataType = ftString
        NumericScale = 255
        Precision = 255
        Size = 5
        Value = '00126'
      end>
    SQL.Strings = (
      'SELECT '
      'ser.idPessoal,'
      'ser.idServidor, '
      'dad.Naturalidade,'
      'car.Descricao as Cargo,'
      'lot.Descricao as Lotacao, '
      'ser.idDesligamento,'
      'des.Descricao as Desligamento'
      ', dad.Pai, dad.Mae'
      ''
      'FROM tbServidor as ser'
      'LEFT JOIN tbDados dad on dad.idPessoal = ser.idPessoal'
      'LEFT JOIN tbCargo car on car.idCargo = ser.idCargo'
      'LEFT JOIN tbLotacao lot on lot.idLotacao = ser.idLotacao'
      
        'LEFT JOIN tbDesligamento des on des.idDesligamento = ser.idDesli' +
        'gamento'
      'WHERE ser.idPessoal =:idPessoal')
    Left = 592
    Top = 8
  end
  object dsServidor: TDataSource
    DataSet = qryServidor
    Left = 624
    Top = 8
  end
  object qryTelefone: TADOQuery
    Connection = DMConexao.conPessoal
    CursorType = ctStatic
    DataSource = dsPessoal
    Parameters = <
      item
        Name = 'idPessoal'
        DataType = ftString
        NumericScale = 255
        Precision = 255
        Size = 5
        Value = '00126'
      end>
    SQL.Strings = (
      'SELECT '
      'tel.idPessoal,'
      'tel.Numero'
      'FROM tbServ_Telefone as tel'
      'WHERE tel.idPessoal =:idPessoal')
    Left = 696
    Top = 8
  end
  object dsTelefone: TDataSource
    DataSet = qryTelefone
    Left = 728
    Top = 8
  end
end
