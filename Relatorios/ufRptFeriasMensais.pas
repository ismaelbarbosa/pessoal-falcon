...
begin
  ...
  sql := ' Set DateFormat DMY ' +
                ...
                'ADI= case when Antecipacao = 0 then ''NÃO'' ' +
                'when Antecipacao = 1 then ''SIM'' ' +
                'end, ' +

                'PER = case when idPeriodo = ''U'' then ''UNI'' ' +
                'when idPeriodo = ''1'' then ''1º'' ' +
                'when idPeriodo = ''2'' then ''2º'' ' +
                'else ''---'' ' +
                'end, ' +
                'PAR = case when idParcela = ''U'' then ''UNI'' ' +
                'when idParcela = ''1'' then ''1ª'' ' +
                'when idParcela = ''2'' then ''2ª'' ' +
                'when idParcela = ''3'' then ''3ª'' ' +                
                'else ''---''end, ' +
...
if rdgrpTipo.ItemIndex = 0 then
   ...
   with dmRelatorios do
     ...
     case rdgrpTipo.ItemIndex of
       ...
       'RELAÇÃO DE FÉRIAS MENSAL'; //Relatório Mensal de Férias';
       end;//0
     end;//case
